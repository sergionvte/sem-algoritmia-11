    ...
    , ui(new Ui::MainWindow)
{
    ...
    tablaNeuronas->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);
}
...
void MainWindow::on_pushButton_clicked()
{
    ...
    for (const Neurona& neurona : neuronas) {
        ...
        qreal centro_x = neurona.get_posicionX() + neurona.get_voltaje() / 2;
        qreal centro_y = neurona.get_posicionY() + neurona.get_voltaje() / 2;

        scene->addEllipse(neurona.get_posicionX(), neurona.get_posicionY(), neurona.get_voltaje(), neurona.get_voltaje(), pen, Bru);

        QPen whitePen(Qt::white); // QPen color blanco
        for (const Neurona& otra_neurona : neuronas) {
            qreal otra_centro_x = otra_neurona.get_posicionX() + otra_neurona.get_voltaje() / 2;
            qreal otra_centro_y = otra_neurona.get_posicionY() + otra_neurona.get_voltaje() / 2;

            // Calcular distancia entre las neuronas
            double distancia = std::sqrt(std::pow(otra_centro_x - centro_x, 2) + std::pow(otra_centro_y - centro_y, 2));

            // Imprimir la distancia en la consola
            std::cout << "Distancia entre Neurona " << neurona.get_id() << " y Neurona " << otra_neurona.get_id() << " = " << std::fixed << std::setprecision(2) << distancia << std::endl;

            //scene->addLine(centro_x, centro_y, otra_centro_x, otra_centro_y, pen); // Lineas de colores
            scene->addLine(centro_x, centro_y, otra_centro_x, otra_centro_y, whitePen); // Lineas blancas
        }
    }
